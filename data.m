classdef data
    ...
    methods
        function obj = data(EEG,start,duration,des_fs,channels,ref)
            obj.fs = EEG.channels.Fs;
            if strcmp(duration,'all'); obj.samples = size(EEG.data,1);
            else; obj.samples = duration*des_fs; end
            ...
            for ch = 1:size(EEG.data,2)
                ...
                if any(strcmp(chan, ["A1","A2"])) || any(strcmp(chan, ["M1","M2"])); obj.mastoids = horzcat(obj.mastoids,EEG.data(start*obj.fs+1:end,ch)); end
                if any(strcmp(chan, ref)); obj.ref_data = EEG.data(start*obj.fs+1:end,ch); end
            end
        end

        % Re-referencing to desired reference
        function data = original_ref(obj)
            data = obj;
        ...
        function data = resampling(obj,des_fs)
            data = obj;
            [p, q] = rat(des_fs/obj.fs);
            data.raw_data = resample(data.raw_data,p,q);
            if ~isempty(data.mastoids); data.mastoids = resample(data.mastoids,p,q); end
            data.fs = des_fs;
        end

        % Data pre-processing
        function obj = preprocess(obj,reref,filter,wind,overlap,t)
            ...
            obj.ppdata = obj.artefact_rejection(filt_data,wind,overlap,t);          % Artefacts rejection and windowing
        end
        ...
        function filtered_data = filter(obj,pp_data,f_highpass,f_lowpass)
            ...
            [b_stop, a_stop] = butter(1,[49/(obj.fs/2),51/(obj.fs/2)],"stop");
            high_flit = filtfilt(b_high,a_high,pp_data);
            filtered_data = filtfilt(b_low,a_low,high_flit);
            if f_highpass <= 50 && f_lowpass >= 50; filtered_data = filtfilt(b_stop,a_stop,filtered_data); end
        end

        % Window-based artefacts rejection
        function wind_data = artefact_rejection(obj,pp_data,window,overlap,t)
            window = floor(window*obj.fs); overlap = floor(overlap*obj.fs);
            ...
            while size(wind_data,3) < nb_wind
                ...
                if any(any(wind>t)) || any(any(wind<-t)) || any(max(wind)-min(wind)<2); w = w + 1;
        ...
        function obj = narrow_filtering(obj,f_bands)
            ...
            filtered_data = zeros([size(extended_data),size(f_bands,1)]);
        ...
        function obj = hanning_windows(obj)
            ...
            hann_wind = repmat(hann(size(extended_data,1),"periodic"),[1 size(extended_data,2,3)]);
        ...
        function [padded_data, padding_size] = padding_windows(obj,data)
            padding_size = floor(size(data,1)/4);
